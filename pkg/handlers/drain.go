...
func isKProbe(r *http.Request) bool {
	return r.Header.Get(pkg.ProbeHeaderName) == pkg.ProbeHeaderValue
}

// ServeHTTP implements http.Handler
func (d *Drainer) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	if pkg.IsKubeletProbe(r) { // Respond to probes regardless of path.
		if d.draining() {
...
// handleProbe is a standalone probe handler.
